...
static int s6e8ax0_adb_brightness_update(struct lcd_info *lcd, u32 br, u32 force)
{
	...
	if ((force) || ((lcd->ldi_enable) && (lcd->current_bl != lcd->bl))) {
		...
		//CTS_Fix
		//printk("Kishore.. brightness=%d, bl = %d, gamma=%d\n", br, lcd->bl, gamma);
...
static int s6e8ax0_set_brightness(struct backlight_device *bd)
{
	...
	//printk("Kishore.. brightness=%d, bl=%d\n", bd->props.brightness, lcd->bl);
...
static DEVICE_ATTR(power_reduce, 0664, acl_enable_show, acl_enable_store);
...
static int s6e8ax0_probe(struct device *dev)
{
	...
	lcd->bd = backlight_device_register("panel", dev,
		lcd, &s6e8ax0_backlight_ops, NULL);
