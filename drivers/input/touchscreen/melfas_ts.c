...
#define	TS_STATE_RELEASE	(0)
#define	TS_STATE_INACTIVE	(-1)
...
//	printk(KERN_ERR "%s : value %d, IDX = %d\n", __func__, lntensity_data[idx], idx);
	...
	if ( idx >= 350)
	{
	printk(KERN_ERR "%s : Resetting refdata IDX now. IDX = %d\n", __func__, idx);
	melfas_power_off();
	mdelay(100);
...
//	printk(KERN_ERR "%s : value %d, idx=%d\n", __func__, inspection_data[idx], idx);
	...
	if ( idx >= 350)
	{
	printk(KERN_ERR "%s : Resetting now. IDX = %d\n", __func__, idx);
	melfas_power_off();
	mdelay(100);
	melfas_init_panel(ts);
	}
...
static void melfas_ts_get_data(struct melfas_ts_data *ts)
{
	...
	if(read_num>0)
	{
		...
		if ((buf[0] & 0x80) && touch_cpu_lock_status == 0)
		{
			...
			/*printk(KERN_DEBUG "[melfasTSP] PreData Pressed melfas_ts_get_data:\
		...
		else if (!(buf[0] & 0x80) && touch_cpu_lock_status)
		{
			...
			/*printk(KERN_DEBUG "[melfasTSP] PreData Release melfas_ts_get_data:\
				Touch ID: %d, State : %d, x: %d, y: %d, z: %d w: %d\n",
				i,(g_Mtouch_info[i].strength > TS_STATE_RELEASE), 
				g_Mtouch_info[i].posX, g_Mtouch_info[i].posY, 
				g_Mtouch_info[i].strength, g_Mtouch_info[i].width);*/
		...
		if(buf[0] == 0x0F)
		{
			...
			{
				g_Mtouch_info[i].strength = TS_STATE_INACTIVE;
	...
	for(i=0; i<(MELFAS_MAX_TOUCH-1); i++)
	{
		if(g_Mtouch_info[i].strength == TS_STATE_INACTIVE)
			continue;

#if	__SUPPORT_SLOT_FOR_ICS__
		if(g_Mtouch_info[i].strength == TS_STATE_RELEASE)
		{
			/* released touch screen */
			input_mt_slot(ts->input_dev, i);
			input_mt_report_slot_state(ts->input_dev, MT_TOOL_FINGER, false);
			/*tsp_dbg_msg("Touch ID: %d, state :%d, x: %d, y: %d,MT_SLOT:false\n",
					i,(g_Mtouch_info[i].strength > TS_STATE_RELEASE), 
					g_Mtouch_info[i].posX, g_Mtouch_info[i].posY);*/
		...
		/* pressed touch screen */
		input_mt_slot(ts->input_dev, i);
		input_mt_report_slot_state(ts->input_dev, MT_TOOL_FINGER, true);
		/*tsp_dbg_msg("Touch ID: %d, state :%d, x: %d, y: %d,MT_SLOT:true_\n",
					i,(g_Mtouch_info[i].strength > TS_STATE_RELEASE), 
					g_Mtouch_info[i].posX, g_Mtouch_info[i].posY);*/
#endif
		...
		if(g_Mtouch_info[i].strength > TS_STATE_RELEASE)		//Kishore
			_touch_is_pressed = 1;
	}
...
// End: Merging from Korean Dali - Sysfs related
	...
	printk(KERN_ERR "[melfasTSP]melfas tsp version: 0x%x 0x%x 0x%x 0x%x\n", buf[0],buf[1],buf[2],buf[3]);			
	...
	if(buf[3] != FW_VERSION || buf[2] != COMPATIBILITY_VERSION)
	{
		...
		printk(KERN_ERR "[melfasTSP] version check error! retry! melfas tsp version: 0x%x 0x%x 0x%x 0x%x\n", buf[0],buf[1],buf[2],buf[3]);			
	}
