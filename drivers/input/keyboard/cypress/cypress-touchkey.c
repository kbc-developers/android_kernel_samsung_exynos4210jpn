...
static int touchkey_keycode[5] = {(int)NULL, KEY_MENU, KEY_BACK, KEY_HOME, (int)NULL};
#else
...
int touchkey_firmware_update_k1(int fw_update_condition)
{
	...
	if(touch_version < K1_FIRMWARE_VERSION || fw_update_condition == 0x18)
	{
		...
		//printk(KERN_DEBUG "[Touchkey] *** Update for FW_Dali_JPN_UNIT_F03M05_20110906\n");
			...
			printk(KERN_DEBUG "[TouchKey FW Download] *** Update for Dali_JPN_UNIT_F04M01_20111026\n");
		update_firmware = true;
	}
			      ...
			      const struct i2c_device_id *id)
{
	...
	set_bit(EV_LED, input_dev->evbit);		// PLM 1898 -- New policy for ICS to be added
	set_bit(LED_MISC, input_dev->ledbit);		// PLM 1898 -- New policy for ICS to be added
...
#elif defined CONFIG_TARGET_LOCALE_NA  /* Touchkey LED command changed from F/W Rev 0xC */
#if defined(CONFIG_MACH_C1_KDDI_REV00) 
	if(data == 1) 
	{
		data = 0x10;
...
static int __init touchkey_init(void)
{
	...
	#if !defined(CONFIG_MACH_C1_KDDI_REV00)
	if (gpio_request(_3_TOUCH_SDA_28V, "_3_TOUCH_SDA_28V") < 0)
		WARN(1, "Fail to request gpio (_3_TOUCH_SDA_28V)\n");
	if (gpio_request(_3_TOUCH_SCL_28V, "_3_TOUCH_SCL_28V") < 0)
		WARN(1, "Fail to request gpio (_3_TOUCH_SCL_28V)\n");
	if (gpio_request(_3_GPIO_TOUCH_EN, "_3_GPIO_TOUCH_EN") < 0)
		WARN(1, "Fail to request gpio (_3_GPIO_TOUCH_EN)\n");
	if (gpio_request(_3_GPIO_TOUCH_INT, "_3_GPIO_TOUCH_INT") < 0)
