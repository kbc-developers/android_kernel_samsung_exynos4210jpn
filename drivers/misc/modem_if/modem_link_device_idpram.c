...
static int idpram_lock_write
(
const char *func,
struct idpram_link_pm_data *pdata
)
{
	int lock_value;

	lock_value =
		(pdata->idpram_wpend == IDPRAM_WPEND_LOCK) ? \
		-3 : atomic_inc_return(&pdata->write_lock);
	if (lock_value != 1)
		pr_info("<%s> lock lock_value (%d) != 1\n", func, lock_value);

	return lock_value;
}

static void idpram_unlock_write
(
const char *func,
struct idpram_link_pm_data *pdata
)
{
	...
	lock_value = atomic_dec_return(&pdata->write_lock);

	if (lock_value != 0)
		pr_info("<%s> unlock lock_value (%d) != 0\n", func, lock_value);
}
...
static int idpram_init_magic_num(struct dpram_link_device *dpld)
{
	...
	if (idpram_lock_write(__func__, dpld->link_pm_data) < 0)
		return -EAGAIN;

	/*write DPRAM disable code */
	dpctl->set_access(acc_code);

	/* write DPRAM magic code :  Normal boot Magic - 0x4D4E*/
	dpctl->set_magic(magic_code);

	/*write enable code */
	acc_code = 0x0001;
	dpctl->set_access(acc_code);

	idpram_unlock_write(__func__, dpld->link_pm_data);

	ret_value = dpctl->get_magic();

	pr_info("[QSC] <%s> Read_MagicNum : 0x%04x\n", __func__, ret_value);
...
)
{
	...
	if (ld->mdm_data->modem_type == QC_QSC6085)
		if (idpram_lock_write(__func__, dpld->link_pm_data) < 0)
			return -EAGAIN;
...
static void cmd_phone_start_handler(struct dpram_link_device *dpld)
{
	...
	if (ld->mdm_data->modem_type == QC_QSC6085) {
		dpld->dpctl->send_intr(INT_CMD(INT_CMD_INIT_START));
		pr_info("[LNK] <%s:%s> Send 0xC1 (INIT_START)\n",
			__func__, ld->name);
	}
	...
	if (ld->mdm_data->modem_type == QC_QSC6085)
		idpram_lock_write(__func__, dpld->link_pm_data);
	...
	} else if (ld->mdm_data->modem_type == QC_QSC6085) {
...
static void idpram_power_down(struct dpram_link_device *dpld)
{
	struct idpram_link_pm_data *pm_data = dpld->link_pm_data;
	pr_info("[QSC] <%s> Received MBX_CMD_DPRAM_DOWN.\n", __func__);
	complete(&pm_data->idpram_down);
}

static void idpram_power_up(struct dpram_link_device *dpld)
{
	struct idpram_link_pm_data *pm_data = dpld->link_pm_data;
	pr_info("[QSC] <%s> Received MBX_CMD_CP_WAKEUP_START\n", __func__);
	complete(&pm_data->idpram_down);
}
...
static void command_handler(struct dpram_link_device *dpld, u16 cmd)
{
	...
	case INT_CMD_DPRAM_DOWN:
		idpram_power_down(dpld);
		break;

	case INT_CMD_CP_WAKEUP_START:
		idpram_power_up(dpld);
		break;
...
static irqreturn_t link_cp_dump_int_handler(int irq, void *arg)
{
	return IRQ_HANDLED;
}

static irqreturn_t link_host_wakeup_handler(int irq, void *data)
{
	struct idpram_link_pm_data *pm_data = data;

	pr_info("[QSC] <%s> wake_lock_timeout() in 5 seconds.\n", __func__);
	wake_lock_timeout(&pm_data->host_wakeup_wlock, 5*HZ);

	return IRQ_HANDLED;
}

static void idpram_clear(struct modemlink_dpram_control *dpctl)
{
	long i = 0;
	long size = 0;
	unsigned long flags;
	...
	size = dpctl->dp_size - (DPRAM_INTERRUPT_PORT_SIZE * 4);

	/*clear DPRAM except interrupt area */
	local_irq_save(flags);

	for (i = DPRAM_PDA2PHONE_FORMATTED_HEAD_ADDRESS; i < size; i += 2)
		iowrite16(0, (void *)(dpctl->dp_base + i));

	local_irq_restore(flags);

	value = dpctl->recv_intr();
}
...
static int idpram_pm_suspend(struct device *dev)
{
	...
	struct idpram_link_pm_data *pm_data = container_of(pdev,
			struct idpram_link_pm_data,
			pdev);
	struct dpram_link_device *dpld = pm_data->idpram_ld;
	struct modemlink_dpram_control *dpctl = dpld->dpctl;

	ld = &dpld->ld;
	iod = find_iodev(ld, IPC_FMT);

	pm_data->idpram_wpend = IDPRAM_WPEND_LOCK; /*dpram write lock*/
	...
	*/
	do {
		...
		dpctl->send_intr(INT_CMD(INT_CMD_PDA_SLEEP));
		timeout_ret = wait_for_completion_timeout(&pm_data->idpram_down,
			msecs_to_jiffies(500));
		pr_info("[QSC] <%s> suspend_enter cnt = %d\n",
			__func__, (DPRAM_PDNINTR_RETRY_CNT - suspend_retry));
	} while (!timeout_ret && suspend_retry--);

	in_intr = dpctl->recv_intr();
	if (in_intr != (INT_MASK_VALID|INT_MASK_CMD|INT_CMD_DPRAM_DOWN)) {
		pr_err("[QSC] <%s> T-I-M-E-O-U-T !!! (intr = 0x%02X)\n",
			__func__, in_intr);
		/* dpram write unlock*/
		pm_data->idpram_wpend = IDPRAM_WPEND_UNLOCK;

		if (iod->mc->phone_state == STATE_BOOTING) {
			pr_err("[QSC] <%s> fail : STATE_BOOTING\n", __func__);
			iod->modem_state_changed(iod, STATE_CRASH_EXIT);
		}
		return -1;
	}
	...
	*/
	...
	dpctl->set_sleep_cfg(0);/*set sleep gpio*/

	if (iod->mc->gpio_pda_active)
		gpio_set_value(iod->mc->gpio_pda_active, 0);

	/*To configure AP_WAKE wakable interrupt*/
	disable_irq(iod->mc->irq_host_wakeup);
	enable_irq_wake(iod->mc->irq_host_wakeup);
...
static int idpram_pm_resume(struct device *dev)
{
	...
	dpctl->set_sleep_cfg(1);/*set wakeup gpio*/
	...
	do {
		init_completion(&pm_data->idpram_down);
		dpctl->send_intr(INT_CMD(INT_CMD_PDA_WAKEUP));
		timeout_ret = wait_for_completion_timeout(&pm_data->idpram_down,
			WAKESTART_TIMEOUT);
	} while (!timeout_ret && wakeup_retry--);

	if (!timeout_ret)
		pr_err("[QSC] <%s>resume T-I-M-E-O-U-T !!!\n", __func__);

	pm_data->idpram_wpend = IDPRAM_WPEND_UNLOCK; /*dpram write unlock*/

	/*To configure AP_WAKE normal interrupt*/
	disable_irq_wake(iod->mc->irq_host_wakeup);
	enable_irq(iod->mc->irq_host_wakeup);
...
)
{
	...
	struct idpram_link_pm_data *pm_data =
			kzalloc(sizeof(struct idpram_link_pm_data), GFP_KERNEL);
	if (!pm_data) {
	...
	pm_data->gpio_link_host_wakeup = pdata->gpio_host_wakeup;
	pm_data->irq_link_host_wakeup =
		gpio_to_irq(pm_data->gpio_link_host_wakeup);

	pm_data->gpio_link_cp_dump_int = pdata->gpio_cp_dump_int;
	pm_data->irq_link_cp_dump_int = gpio_to_irq(pdata->gpio_cp_dump_int);

	pm_data->idpram_ld = idpram_ld;
	idpram_ld->link_pm_data = pm_data;

	pm_data->pdev.name = "idparam_pm";

	init_completion(&pm_data->idpram_down);
	wake_lock_init(&pm_data->host_wakeup_wlock,
		WAKE_LOCK_SUSPEND,
		"HOST_WAKEUP_WLOCK");

	r = platform_device_register(&pm_data->pdev);
	if (r < 0) {
	...
	r = request_irq(pm_data->irq_link_host_wakeup, link_host_wakeup_handler,
		IRQF_TRIGGER_RISING,
		"idpram_host_wakeup", (void *)pm_data);

	pr_info("[LNK] <%s> DPRAM IRQ# = %d, %d\n", __func__,
		pm_data->irq_link_host_wakeup,
		pm_data->gpio_link_host_wakeup);

	if (r) {
		pr_err("%s:fail to request irq(%d) host_wake_irq\n",
			__func__, r);
		goto err_request_irq;
	}

	r = enable_irq_wake(pm_data->irq_link_host_wakeup);
	if (r) {
		pr_err("%s: failed to enable_irq_wake:%d host_wake_irq\n",
			__func__, r);
		goto err_set_wake_irq;
	}

	r = request_irq(pm_data->irq_link_cp_dump_int, link_cp_dump_int_handler,
		IRQF_TRIGGER_RISING,
		"idpram_cp_dump_int", (void *)pm_data);
	pr_info("[LNK] <%s> DPRAM IRQ# = %d, %d\n", __func__,
		pm_data->irq_link_cp_dump_int,
		pm_data->gpio_link_cp_dump_int);
	if (r) {
		pr_err("%s:fail to request irq(%d) cp_dump_int\n", __func__, r);
		goto err_cp_cump_request_irq;
	}

	r = enable_irq_wake(pm_data->irq_link_cp_dump_int);
	if (r) {
		pr_err("%s: failed to enable_irq_wake:%d cp_dump_int\n",
			__func__, r);
		goto err_set_cpdump_irq;
	}
...
err_set_cpdump_irq:
	free_irq(pm_data->irq_link_cp_dump_int, (void *)pm_data);
err_cp_cump_request_irq:
	disable_irq(pm_data->irq_link_host_wakeup);
err_set_wake_irq:
	free_irq(pm_data->irq_link_host_wakeup, (void *)pm_data);
err_request_irq:
	platform_driver_unregister(&idpram_pm_driver);
err_platform_driver_register:
	platform_device_unregister(&pm_data->pdev);
...
struct link_device *dpram_create_link_device(struct platform_device *pdev)
{
	...
	dpld->init_magic_num = idpram_init_magic_num;
