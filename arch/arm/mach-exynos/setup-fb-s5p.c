		...
		unsigned int cfg, s5p_gpio_drvstr_t drvstr)
{
	...
	printk("Kishore inside s3cfb_gpio_setup_24bpp start = %d, size = %d, cfg = %d, drvstr = %d\n", start, size, cfg, drvstr);
...
int s3cfb_backlight_on(struct platform_device *pdev)
{
	printk("Kishore inside s3cfb_backlight_on \n");
	return 0;
}

int s3cfb_backlight_off(struct platform_device *pdev)
{
	printk("Kishore inside s3cfb_backlight_off \n");
	return 0;
}

int s3cfb_lcd_on(struct platform_device *pdev)
{
	printk("Kishore inside s3cfb_lcd_on \n");
	return 0;
}

int s3cfb_lcd_off(struct platform_device *pdev)
{
	printk("Kishore inside s3cfb_lcd_off \n");
	return 0;
}
#endif
...
int s3cfb_mdnie_clk_on(u32 rate)
{
	...
	printk("Kishore inside s3cfb_mdnie_clk_on \n");
...
int s3cfb_mdnie_pwm_clk_on(void)
{
	...
	printk("Kishore inside s3cfb_mdnie_pwm_clk_on \n");
...
int s3cfb_clk_on(struct platform_device *pdev, struct clk **s3cfb_clk)
{
	...
	printk("Kishore inside s3cfb_clk_on \n");
...
int s3cfb_mdnie_clk_off(void)
{
	...
	printk("Kishore inside s3cfb_mdnie_clk_off \n");
	mdnie_clk = clk_get(NULL, "mdnie0"); /*   CLOCK GATE IP ENABLE */
	if (IS_ERR(mdnie_clk)) {
...
int s3cfb_mdnie_pwm_clk_off(void)
{
	...
	printk("Kishore inside s3cfb_mdnie_pwm_clk_off \n");
	sclk = clk_get(NULL, "sclk_mdnie_pwm");
	if (IS_ERR(sclk))
		printk(KERN_ERR "failed to get sclk for mdnie_pwm\n");
...
int s3cfb_clk_off(struct platform_device *pdev, struct clk **clk)
{
	...
	printk("Kishore inside s3cfb_clk_off \n");
	lcd_clk = clk_get(&pdev->dev, "lcd");
	if (IS_ERR(lcd_clk)) {
		printk(KERN_ERR "failed to get ip clk for fimd0\n");
		goto err_clk0;
