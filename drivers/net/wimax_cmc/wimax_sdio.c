...
long uwbrdev_ioctl(struct file *file, u_int cmd, u_long arg)
{
	struct net_adapter		*adapter;
	struct process_descriptor	*process;
	int				ret = 0;
...
{
	dump_debug("adapter_suspend sdio");
	return 0;
}

static int adapter_resume(struct device *dev)
{
	dump_debug("adapter_resume sdio");
	return 0;
}
...
void adapter_interrupt(struct sdio_func *func)
{
	...
	if (!gpio_get_value(GPIO_WIMAX_CON1)) {
		dump_debug("CON1 not set high");
		return;
	}
